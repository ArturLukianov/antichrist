...
class NPC(Actor):
            ...
            if self.relations.get(actor.name, None) is None:
                ...
                for item in actor.inventory:
                    if item.item_type == 'weapon':
                        if item.equipped:
                            self.change_relations(actor.name, -10)
                        else:
                            self.change_relations(actor.name, -3)
    ...
    def define_current_action(self, ticks):
        ...
        if self.current_action == 'kill':
            if self.target.level != self.level or self.target is None or not self.target.is_alive or self.relations[self.target.name] >= 0 or self.target.pos.distance(self.pos) > 20:
                self.current_action = None
